...
// تحديث الكمية في المخزون بعد الشراء
foreach ($_SESSION['bill'] as $row) {
    $productId = $row['id']; // تأكد أن `id` يمثل معرف المنتج
    $quantityPurchased = $row['qty'];

    // تحديث الكمية في قاعدة البيانات
    if (!$con->query("UPDATE inventeries SET quantity = quantity - $quantityPurchased WHERE id = '$productId'")) {
        echo "Error updating quantity for product ID $productId: " . $con->error;
    }
}
